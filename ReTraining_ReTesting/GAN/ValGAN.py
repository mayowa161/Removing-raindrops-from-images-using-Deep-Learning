...
def resize_np_images(images, new_size=(128,128)):
    """Given a 4D NumPy array of shape (N, H, W, C),
       resize each image to new_size=(height, width)."""
    resized_list = []
    for i in range(images.shape[0]):
        pil_img = Image.fromarray(images[i])     # Convert array -> PIL
        pil_img = pil_img.resize(new_size, Image.BICUBIC)
        resized_list.append(np.array(pil_img))   # Convert PIL -> array
...
rain_images_a = resize_np_images(rain_images_a, (128,128))
...
rain_images_b = resize_np_images(rain_images_b, (128,128))
...
clean_images_a = resize_np_images(clean_images_a, (128,128))
...
clean_images_b = resize_np_images(clean_images_b, (128,128))
...
mask_images = resize_np_images(mask_images, (128,128))
...
# Example: saving initial weights if needed
# generator.save_weights(filepath = '/users/lady6758/.../generatorinitialweights.weights.h5')
# discriminator.save_weights(filepath = '/users/lady6758/.../discriminatorinitialweights.weights.h5')
...
for i, (train_index, val_index) in enumerate(kf.split(clean_images)):
    ...
    # Example skipping folds 0 and 1
    # if i == 0:
    #     continue
    # if i == 1:
    #     continue
    ...
    np.save(f'/users/lady6758/Mayowa_4YP_Code/ReTraining_ReTesting/GAN/5FoldCV/model{i}/val.npy', np.asarray(val_index))
    ...
    # --> Removed 'vgg' since we won't compute it now
  ...
  #Define the fit step
    ...
    # Save final weights
    generator.save_weights(f'/users/lady6758/Mayowa_4YP_Code/ReTraining_ReTesting/GAN/5FoldCV/model{i}/generator_weights.weights.h5')
    discriminator.save_weights(f'/users/lady6758/Mayowa_4YP_Code/ReTraining_ReTesting/GAN/5FoldCV/model{i}/discriminator_weights.weights.h5')

    # Save final dictionary
    with open(f'/users/lady6758/Mayowa_4YP_Code/ReTraining_ReTesting/GAN/5FoldCV/model{i}/loss.pkl', 'wb') as fp:
        pickle.dump(my_dict, fp)
        print('dictionary saved successfully to file')
