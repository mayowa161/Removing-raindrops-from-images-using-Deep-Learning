...
for i in range(num_folds):
    ...
    best_weights_path = os.path.join(model_folder_paths, f"model{i}", "modelweights_val.pth")
    if not os.path.exists(best_weights_path):
        print(f"Fold {i} best weights not found at {best_weights_path}, skipping.")
        continue

    # Initialize the model and load weights
    fold_model = EndToEndModel().to(device)
    fold_model.load_state_dict(torch.load(best_weights_path, map_location=device))
    fold_model.eval()
    ...
    with torch.no_grad():
        for (rain_img, clean_img, true_mask) in val_loader:
            ...
            gen_mask, ms_output1, ms_output2, ms_output3 = fold_model(rain_img)
            ...
            # PSNR
            batch_gen_psnr = psnr(ms_output1, clean_img, max_val=1.0)
            # SSIM
            batch_gen_ssim = ssim(ms_output1, clean_img)
