...
class Generator(nn.Module):
    ...
    def __init__(self, inX_chs=3, inRM_chs=1, inED_chs=1, out_chs=3, nfeats=32, num_main_blk=1,
    ...
    def forward(self, x, rm=None, ed=None):
        if rm is not None or ed is not None:
            ...
            residual = self.out(self.auam((fea_x, fea_sub))) 
        else:
            fea_x = self.aam(x)
            for main in self.main_blk:
                fea_x = main(fea_x)
            residual = self.out(fea_x) 
        
        out = torch.clamp(residual + x, 0, 1)
        return out

class Discriminator(nn.Module):
    def __init__(self, inX_chs=3, nfeats=32, inRM_chs=1, inED_chs=1, act_type='DAF',):
        ...
        self.main = nn.Sequential(
           spectral_norm(nn.Conv2d(nfeats, nfeats, kernel_size=5, stride=2, padding=2)),
            act,
            spectral_norm(nn.Conv2d(nfeats, nfeats * 2, kernel_size=5, padding=2)),
            act,
            spectral_norm(nn.Conv2d(nfeats * 2, nfeats * 2, kernel_size=5, stride=2, padding=2)),
            act,
            spectral_norm(nn.Conv2d(nfeats * 2, nfeats * 4, kernel_size=5, padding=2)),
            act,
            spectral_norm(nn.Conv2d(nfeats * 4, nfeats * 4, kernel_size=5, stride=2, padding=2)),
            act,
            nn.AdaptiveAvgPool2d(1),
            spectral_norm(nn.Conv2d(nfeats * 4, nfeats * 8, kernel_size=1)),
            act,
            spectral_norm(nn.Conv2d(nfeats * 8, 1, kernel_size=1)),
        )
    ...
    def forward(self, x, rm=None, ed=None):
        ...
        return torch.sigmoid(si)


### sub class
class AAM(nn.Module):

    def __init__(self, inX_chs=3, nfeats=32, inRM_chs=1, inED_chs=1,
